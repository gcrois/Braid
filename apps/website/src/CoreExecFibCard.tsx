...
import { CCoreWorker, CoreData, RustCoreWorker, useCoreContext } from "./coreContext";
			...
			{cores.map((core) => {
				...
				return (
					<div key={core.name} className="core-exec-row">
						...
						{hasFib ? (
							<div className="row">
								<button onClick={() => runFib(core.name, convertToFibModule(core))}>
                                    Run Fib
                                </button>
								<button onClick={() => benchmarkFib(core.name, convertToFibModule(core))}>
									Benchmark Fib
								</button>
								{results[core.name] && (
									<div className="result">
										Result: {results[core.name].value} <br />
										Time: {results[core.name].time.toFixed(2)} ms
									</div>
								)}
							</div>
						) : (
